...
class ItemListAdapter(
    ...
    var heroList: ArrayList<ItemListData>, id: Int,
    private var listClickListener: ListClickListener,

    ) : ArrayAdapter<ItemListData?>(
    context, resource, heroList as List<ItemListData?>
), Filterable {
    private var selectedId: Int
    var tmpArray: ArrayList<ItemListData> = heroList
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (selectedId == hero.itemid!!.toInt()) {
...
//        )
    ...
    override fun getItem(position: Int): ItemListData? {
    ...
    inner class CustomeFilter : Filter() {
        override fun performFiltering(constraints: CharSequence): FilterResults {
            ...
            if (constraints != null && constraints.length > 1) {
                ...
                for (i in tmpArray.indices) {
                    if (tmpArray[i].itemname!!.lowercase(Locale.getDefault()).contains(constraints)) {
                        val gson = Gson()
        ...
        override fun publishResults(charSequence: CharSequence, filterResults: FilterResults) {
            heroList = filterResults.values as ArrayList<ItemListData>
